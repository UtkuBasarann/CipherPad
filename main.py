...
def save_and_encrypt_notes():
    ...
    if len(title) == 0 or len(message) == 0 or len(master_secret) == 0:
        ...
        try:
            with open("CipherText.txt", "a") as data_file:
                data_file.write(f'\n{title}\n{message_encrypted}')
        except FileNotFoundError:
            with open("CipherText.txt", "w") as data_file:
                data_file.write(f'\n{title}\n{message_encrypted}')
        finally:
...
#Interface
window = Tk()
window.title("CipherPad")
window.config(padx=30, pady=30)

canvas = Canvas(height=200, width=200)
logo = PhotoImage(file="CipherPad.png")
canvas.create_image(100,100,image=logo)
canvas.pack()

title_info_label = Label(text="Title",font=("Century Gothic",20,"normal"))
title_info_label.pack()

title_entry = Entry(width=30)
title_entry.pack()

input_info_label = Label(text="Text",font=("Century Gothic",20,"normal"))
input_info_label.pack()

input_text = Text(width=50, height=25)
input_text.pack()

master_secret_label = Label(text="Master Key",font=("Century Gothic",15,"normal"))
